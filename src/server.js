...
if (body.list_type === ‘**new**’ || customType) {
if (!customType) {
...
`UPDATE campaign_contact_phones ccp SET wrong_number = true, updated_at = NOW() FROM campaign_numbers cn WHERE cn.phone_number = ccp.phone_number AND cn.campaign_id = ccp.campaign_id AND cn.campaign_id = $1 AND cn.last_disposition_normalized = 'wrong_number' AND ccp.wrong_number = false`,
[req.params.id]
);
...
const rows = display.map(c => ` <tr onclick="location.href='/campaigns/${c.id}'" style="cursor:pointer"> <td><strong>${c.name}</strong><br><span style="font-size:11px;color:#888">${c.list_type} · ${c.market_name}</span></td> <td><span class="badge" style="background:${STATUS_COLORS[c.status]}20;color:${STATUS_COLORS[c.status]}">${c.status}</span></td> <td><span class="badge" style="background:#e6f1fb;color:#185fa5">${CHANNEL_LABELS[c.active_channel]||c.active_channel}</span></td> <td style="font-size:12px">${c.start_date ? new Date(c.start_date).toLocaleDateString() : '—'}</td> <td style="font-size:12px;color:#888">${c.end_date ? new Date(c.end_date).toLocaleDateString() : '—'}</td> <td>${Number(c.total_unique_numbers||0).toLocaleString()}</td> <td style="color:#1a7a4a">${Number(c.total_callable||0).toLocaleString()}</td> <td style="color:#c0392b">${Number(c.total_filtered||0).toLocaleString()}</td> <td>${c.upload_count||0}</td> </tr>`).join(’’);

return shell(‘Campaigns’, `<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1.5rem"> <div><h2 style="font-size:20px;font-weight:500;margin-bottom:3px">Campaigns</h2> <p style="font-size:13px;color:#888">Each campaign tracks all filtration activity for a list type in a market</p></div> <a href="/campaigns/new" class="btn-primary-link">+ New Campaign</a> </div> <div style="display:flex;gap:2px;border-bottom:1px solid #e0dfd8;margin-bottom:1.25rem"> <a href="/campaigns?tab=active" style="padding:8px 16px;font-size:13px;text-decoration:none;border-bottom:2px solid ${tab==='active'?'#1a1a1a':'transparent'};color:${tab==='active'?'#1a1a1a':'#888'}">Active (${active.length})</a> <a href="/campaigns?tab=completed" style="padding:8px 16px;font-size:13px;text-decoration:none;border-bottom:2px solid ${tab==='completed'?'#1a1a1a':'transparent'};color:${tab==='completed'?'#1a1a1a':'#888'}">Completed (${completed.length})</a> </div> ${display.length === 0 ?`<div class="empty-state">No ${tab} campaigns yet.</div>`:`
...
return shell(‘New Campaign’, `<div style="max-width:520px"> <div style="margin-bottom:1.5rem"><a href="/campaigns" style="font-size:13px;color:#888;text-decoration:none">← Campaigns</a></div> <h2 style="font-size:20px;font-weight:500;margin-bottom:4px">New campaign</h2> <p style="font-size:13px;color:#888;margin-bottom:1.5rem">Create a campaign to start tracking filtration activity</p> ${error ?`<div class="error-box">${error}</div>` : ''} <div class="card"> <form method="POST" action="/campaigns/new"> <div class="form-field"> <label>Campaign name</label> <input type="text" name="name" placeholder="e.g. Vacant Property Indiana 2026" required> <span class="field-hint">Use a clear name — this is what you'll select when uploading filtration files</span> </div> <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px"> <div class="form-field"> <label>List type</label> <select name="list_type" id="list_type_select" onchange="document.getElementById('custom_lt_wrap').style.display=this.value==='__new__'?'block':'none';document.getElementById('custom_lt_input').required=this.value==='__new__'"> <option value="">Select...</option> ${LIST_TYPES.map(t=>`<option value="${t}">${t}</option>`).join('')} <option value="__new__">+ Add new list type…</option> </select> <div id="custom_lt_wrap" style="display:none;margin-top:8px"> <input type="text" id="custom_lt_input" name="custom_list_type" placeholder="Enter new list type" maxlength="100"> <span class="field-hint">Saved for future campaigns</span> </div> </div> <div class="form-field"> <label>State</label> <select name="state_code" required> <option value="">Select...</option> ${STATES.map(s=>`<option value="${s}">${s}</option>`).join('')} </select> </div> </div> <div class="form-field"> <label>Market name</label> <input type="text" name="market_name" placeholder="e.g. Indianapolis Metro" required> </div> <div class="form-field"> <label>Start date</label> <input type="date" name="start_date" value="${new Date().toISOString().split('T')[0]}"> </div> <div class="form-field"> <label>Active channel</label> <select name="active_channel"> <option value="cold_call">Cold Call</option> <option value="sms">SMS</option> </select> </div> <div class="form-field"> <label>Notes (optional)</label> <textarea name="notes" rows="2" placeholder="Any notes about this campaign..."></textarea> </div> <button type="submit" class="btn-submit">Create campaign</button> </form> </div> </div> `);
}
...
const html = entries.map(e => `<div class="card" style="margin-bottom:1.5rem"> <div style="border-bottom:1px solid #f0efe9;padding-bottom:10px;margin-bottom:14px"> <div style="font-size:12px;color:#888;text-transform:uppercase;letter-spacing:0.5px">${e.date}</div> <div style="font-size:16px;font-weight:500;margin-top:2px">${e.title}</div> </div> ${e.items.map(i => { const c = tagColors[i.tag] || { bg: '#f0efe9', color: '#888' }; return`<div style="display:flex;gap:10px;align-items:flex-start;padding:8px 0">
...
return shell(‘Changelog’, `<div style="max-width:760px"> <h2 style="font-size:20px;font-weight:500;margin-bottom:4px">Changelog</h2> <p style="font-size:13px;color:#888;margin-bottom:1.5rem">Track what's new and what's been fixed in Loki.</p> ${html} </div>`, ‘changelog’);
}
...
const uploadRows = (c.uploads||[]).map(u => ` <tr> <td style="font-size:11px;color:#888">${new Date(u.uploaded_at).toLocaleDateString()} ${new Date(u.uploaded_at).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}</td> <td>${u.filename||'—'}<br><span style="font-size:11px;color:#888">${u.source_list_name||''}</span></td> <td><span class="badge" style="background:#e6f1fb;color:#185fa5">${CHANNEL_LABELS[u.channel]||u.channel}</span></td> <td>${u.total_records}</td> <td style="color:#1a7a4a">${u.records_kept}</td> <td style="color:#c0392b">${u.records_filtered}</td> <td style="color:#888;font-size:11px">WN:${u.wrong_numbers} VM:${u.voicemails} NI:${u.not_interested} DNC:${u.do_not_call} Lead:${u.transfers}</td> <td style="color:#2471a3;font-size:11px">${u.caught_by_memory} by memory</td> <td> <form method="POST" action="/campaigns/${c.id}/uploads/${u.id}/delete" onsubmit="return confirm('Remove this upload from the campaign? This will reverse its counts from memory.')"> <button type="submit" style="background:none;border:none;color:#c0392b;font-size:11px;cursor:pointer;text-decoration:underline;font-family:inherit;padding:0">Remove</button> </form> </td> </tr>`).join(’’);

const dispositionRows = (c.disposition_breakdown||[]).map(d => ` <tr><td>${d.disposition||'unknown'}</td><td style="font-weight:500">${Number(d.count).toLocaleString()}</td></tr>`).join(’’);

return shell(c.name, `<div style="margin-bottom:1rem"><a href="/campaigns" style="font-size:13px;color:#888;text-decoration:none">← Campaigns</a></div> <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:1.5rem;flex-wrap:wrap;gap:12px"> <div> <div style="display:flex;align-items:center;gap:10px;margin-bottom:4px"> <h2 style="font-size:20px;font-weight:500">${c.name}</h2> <span class="badge" style="background:${STATUS_COLORS[c.status]}20;color:${STATUS_COLORS[c.status]}">${c.status}</span> </div> <p style="font-size:13px;color:#888">${c.list_type} · ${c.market_name} · ${c.state_code} · Started ${c.start_date ? new Date(c.start_date).toLocaleDateString() : '—'} ${c.end_date ? '· Ended ' + new Date(c.end_date).toLocaleDateString() : ''} · ${c.upload_count} uploads</p> </div> <div style="display:flex;gap:8px;flex-wrap:wrap"> <form method="POST" action="/campaigns/${c.id}/channel" style="display:inline"> <select name="channel" onchange="this.form.submit()" class="inline-select"> <option ${c.active_channel==='cold_call'?'selected':''} value="cold_call">Cold Call active</option> <option ${c.active_channel==='sms'?'selected':''} value="sms">SMS active</option> </select> </form> ${c.status !== 'completed' ?`
