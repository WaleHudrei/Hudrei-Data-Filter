...
function campaignDetailPage(c) {
  const callable_pct = c.total_unique_numbers > 0 ? Math.round((c.total_callable / c.total_unique_numbers) * 100) : 0;
  const filtered_pct = c.total_unique_numbers > 0 ? Math.round((c.total_filtered / c.total_unique_numbers) * 100) : 0;
  ...
  return shell(c.name, `
    ...
    <div class="stats-grid-5" style="margin-bottom:1.25rem">
      <div class="stat-card"><div class="stat-lbl">Unique numbers</div><div class="stat-num">${Number(c.total_unique_numbers||0).toLocaleString()}</div></div>
      <div class="stat-card"><div class="stat-lbl">Callable</div><div class="stat-num green">${Number(c.total_callable||0).toLocaleString()} <span style="font-size:13px;font-weight:400;color:#888">(${callable_pct}%)</span></div></div>
      <div class="stat-card"><div class="stat-lbl">Filtered out</div><div class="stat-num red">${Number(c.total_filtered||0).toLocaleString()} <span style="font-size:13px;font-weight:400;color:#888">(${filtered_pct}%)</span></div></div>
      <div class="stat-card"><div class="stat-lbl">Transfers (leads)</div><div class="stat-num blue">${Number(c.total_transfers||0).toLocaleString()}</div></div>
      <div class="stat-card"><div class="stat-lbl">Filtration runs</div><div class="stat-num">${c.upload_count||0}</div></div>
    </div>
...
.topbar{background:#1a1a1a;padding:0 2rem;height:52px;display:flex;align-items:center;justify-content:space-between;position:sticky;top:0;z-index:10}
.brand{font-size:14px;font-weight:600;color:#fff}.brand span{color:#666;font-weight:400}
.ver{font-size:11px;background:#2a2a2a;padding:2px 8px;border-radius:4px;color:#888;margin-left:8px}
.nav{display:flex;align-items:center;gap:4px}
.nav a{color:#888;font-size:13px;text-decoration:none;padding:5px 10px;border-radius:6px}
...
<div class="topbar">
  <div style="display:flex;align-items:center;gap:8px">
    <span class="brand">HudREI LLC <span>/ Data Filter</span></span>
    <span class="ver">v2.0</span>
  </div>
  <div class="nav">
    <a href="/" ${title==='List Filtration Bot'?'class="active"':''}>Filtration</a>
    <a href="/campaigns" ${title==='Campaigns'||title.includes('Campaign')||title.includes('Vacant')||title.includes('Foreclosure')||title.includes('Lien')||title.includes('Mortgage')||title.includes('Property')||title.includes('Owner')?'class="active"':''}>Campaigns</a>
