...
router.post('/export', requireAuth, async (req, res) => {
  try {
    ...
    if (selectAll) {
      ...
      if (qv('min_equity_value')) { conditions.push(`p.estimated_equity_value >= $${idx}`); params.push(qv('min_equity_value')); idx++; }
      if (qv('max_equity_value')) { conditions.push(`p.estimated_equity_value <= $${idx}`); params.push(qv('max_equity_value')); idx++; }
