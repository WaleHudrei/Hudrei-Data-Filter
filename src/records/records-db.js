...
async function getProperties({ search = '', page = 1, pageSize = 50 }) {
  ...
  const rows = await query(
    `SELECT
       p.id, p.property_address, p.property_city, p.property_state, p.property_zip,
       COALESCE(
         (SELECT STRING_AGG(DISTINCT TRIM(COALESCE(o.first_name,'') || ' ' || COALESCE(o.last_name,'')), ', ')
          FROM rec_property_owners po
          JOIN rec_owners o ON o.id = po.owner_id
          WHERE po.property_id = p.id),
         '—'
       ) as owners,
       (SELECT COUNT(DISTINCT ccp.phone_number)
        FROM campaign_contacts cc
        JOIN campaign_contact_phones ccp ON ccp.contact_id = cc.id
        WHERE LOWER(cc.property_address) = LOWER(p.property_address)
          AND LOWER(COALESCE(cc.property_zip,'')) = LOWER(COALESCE(p.property_zip,''))
       ) as phone_count,
       (SELECT COUNT(*) FROM rec_property_campaigns pc WHERE pc.property_id = p.id) as list_count
     FROM rec_properties p
     ${whereClause}
     ORDER BY p.id DESC
     LIMIT ${limitParams}`,
    params
