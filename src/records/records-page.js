...
const rowsHtml = properties.map(p => `<tr> <td style="font-weight:500;color:#1a1a1a">${escapeHtml(p.property_address||'')}</td> <td>${escapeHtml(p.property_city||'')}</td> <td>${escapeHtml(p.property_state||'')}</td> <td>${escapeHtml(p.property_zip||'')}</td> <td>${escapeHtml(p.owners||'—')}</td> <td style="text-align:center">${p.phone_count||0}</td> <td style="text-align:center">${p.list_count||0}</td> </tr>`).join(’’);

const emptyState = `<tr><td colspan="7" style="text-align:center;padding:40px;color:#888;font-size:13px"> ${search ? 'No properties match your search.' : 'No properties yet. Click <strong>Sync from campaigns</strong> above to import from your existing contact lists.'} </td></tr>`;
