...
async function _renderExecDashboard(req, res) {
  try {
    ...
    const r = await query(`
      SELECT
        (SELECT COUNT(*)::int FROM properties WHERE tenant_id = $1) AS total_properties,
        (SELECT COUNT(*)::int FROM contacts   WHERE tenant_id = $1) AS total_owners,
        (SELECT COUNT(*)::int FROM phones     WHERE tenant_id = $1) AS total_phones,
        (SELECT COUNT(*)::int FROM phones     WHERE tenant_id = $1 AND phone_status = 'correct')                       AS correct_phones,
        (SELECT COUNT(*)::int FROM phones     WHERE tenant_id = $1 AND (phone_status = 'wrong' OR wrong_number = TRUE)) AS wrong_phones,
        (SELECT COUNT(*)::int FROM phones     WHERE tenant_id = $1 AND phone_status IN ('dead','dead_number'))          AS dead_phones,
        (SELECT COUNT(*)::int FROM phones     WHERE tenant_id = $1 AND do_not_call = TRUE)                              AS dnc_phones,
        (SELECT COUNT(*)::int FROM properties WHERE tenant_id = $1 AND pipeline_stage = 'lead')                         AS lead_props,
        (SELECT COUNT(*)::int FROM properties WHERE tenant_id = $1 AND pipeline_stage = 'contract')                     AS contract_props,
        (SELECT COUNT(*)::int FROM properties WHERE tenant_id = $1 AND pipeline_stage = 'closed')                       AS closed_props,
        (SELECT COUNT(*)::int FROM campaigns  WHERE tenant_id = $1 AND status = 'active')                               AS active_campaigns,
        (SELECT COALESCE(SUM(COALESCE(estimated_value, assessed_value, 0)), 0)::numeric
            FROM properties WHERE tenant_id = $1 AND pipeline_stage IN ('lead','contract','closed'))                    AS pipeline_value,
        (SELECT COUNT(*)::int FROM call_logs  WHERE tenant_id = $1 AND call_date >= CURRENT_DATE - INTERVAL '30 days')  AS calls_30d,
        (SELECT COUNT(*)::int FROM call_logs  WHERE tenant_id = $1 AND call_date >= CURRENT_DATE - INTERVAL '30 days' AND disposition_normalized = 'transfer') AS transfers_30d
    `, [tenantId]);
...
//     trend sparkline (last 8 weeks). Joining via campaign NAME because
...
async function _renderAnalyticsDashboard(req, res) {
  try {
    ...
    // Lead counts per campaign — distinct campaign_contacts flagged 'Lead'.
    const leadsRes = await query(
      `SELECT campaign_id, COUNT(DISTINCT id)::int AS leads
         FROM campaign_contacts
        WHERE tenant_id = $1
          AND campaign_id = ANY($2::int[])
          AND marketing_result = 'Lead'
        GROUP BY campaign_id`,
      [req.tenantId, ids]
    );
    const leadsByCamp = new Map(leadsRes.rows.map(r => [r.campaign_id, Number(r.leads)]));

    // Weekly leads trend — last 8 weeks per campaign, indexed by name. Uses
    // call_logs as the time-series source since campaign_uploads doesn't
    // record the lead outcome.
    const trendRes = await query(
      `SELECT cl.campaign_name,
              date_trunc('week', cl.call_date)::date AS week_start,
              COUNT(*)::int AS count
         FROM call_logs cl
        WHERE cl.tenant_id = $1
          AND cl.disposition_normalized = 'transfer'
          AND cl.call_date >= CURRENT_DATE - INTERVAL '8 weeks'
          AND cl.campaign_name IS NOT NULL
        GROUP BY cl.campaign_name, week_start
        ORDER BY cl.campaign_name, week_start`,
      [req.tenantId]
    );
    const weeklyByName = new Map();
    for (const r of trendRes.rows) {
      const arr = weeklyByName.get(r.campaign_name) || [];
      arr.push({ week_start: r.week_start, count: r.count });
      weeklyByName.set(r.campaign_name, arr);
    }

    const enriched = list.map(c => ({
      ...c,
      leads:        leadsByCamp.get(c.id) || 0,
      weekly_leads: weeklyByName.get(c.name) || [],
    }));
